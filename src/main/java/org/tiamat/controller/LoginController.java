...
import org.tiamat.codeutil.IVerifyCodeGen;
import org.tiamat.codeutil.SimpleCharVerifyCodeGenImpl;
import org.tiamat.codeutil.VerifyCode;
    ...
    public String loginIn(HttpServletRequest request, Model model) {
        ...
        if (!realCode.equalsIgnoreCase(code)) {
            ...
            if (type.equals("1")) {//管理员信息
                //用户名和密码是否正确
                Admin admin = adminService.queryAdminByNameAndPassword(username, password);
                ...
                Employee employee = employeeService.queryEmployeeByNameAndPassword(username, password);
                if (employee == null) {
                    model.addAttribute("msg", "用户名或密码错误");
                    return "login";
                }
